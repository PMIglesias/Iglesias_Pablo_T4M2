...
function init() {
  ...
  function getUpEvents(arrayEventos, currentDate) {
  ...
  getUpEvents(arrayDatos, currentDate);

  function generateCards(upEvents) {
      let tarjetasGeneradas = ""
      for (const evento of upEvents) {
          tarjetasGeneradas += `
  ...
  function generateCheckboxes(upEvents) {
      let categorias = [];
      upEvents.forEach((evento) => {
  ...
  function filterEvents() {
      ...
      if (selectedCategories.length > 0) {
        filteredEvents = upEvents.filter(
          (evento) => selectedCategories.includes(evento.category)
        );
      } else {
        filteredEvents = upEvents;
      }
